...
void main() {
  ...
  test('testSimpleCircleClip', () async {
    var options = AutosuggestOptions()
        .setClipToCircle(Coordinates(-90.000000, 360.0), 100);
    var autosuggest =
  ...
  test('testSimpleCircleClipLatCannotWrap', () async {
    var options = AutosuggestOptions()
        .setClipToCircle(Coordinates(-91.000000, 360.0), 100);
    var autosuggest =
        await api.autosuggest('index.home.ra', options: options).execute();
  ...
  test('testSimpleCircleClipLatBigDistance', () async {
    var options = AutosuggestOptions()
        .setClipToCircle(Coordinates(0.000000, 0.0), 10000000);
    var autosuggest =
        await api.autosuggest('index.home.ra', options: options).execute();
    expect(autosuggest.isSuccessful(), true);
  });
  ...
  test('clipToCountryThatDoesNotExist', () async {
    ...
    var autosuggest =
        await api.autosuggest('index.home.raf', options: options).execute();
